...
switch($action){
	...
	case "change" :
			...
			$sql="select password from teacher where tnumber='$tnumber' and password='$old_pass'";
			$check=db_query($sql);
			if(!empty($check)){
				$sql = "update teacher set password='$new_pass' where tnumber='$tnumber'";
				$isUp = db_update($sql) ;
				if($isUp){
					echo  "<script>					
									window.location.href='system_action.php?action=list&&tnumber=".$tnumber."';											
									alert('密码修改成功！');								
									</script>"
											;
				}else{
