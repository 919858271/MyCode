...
switch($action){//功能选择
	case "list" ://文件列表
		...
		if(!empty($data)){	
			...
			$smarty->assign("course_id",$id);//课程编号
			$smarty->assign("snumber",$snumber);//学号编号
		...
		case "upload" ://资源上传
			...
			$snumber = $_GET['snumber'];//学号
			$sqlt="select sname from student where snumber='$snumber'";
			$sn=db_query($sqlt);			
			$s_name = $sn[0][0];//谁共享的			
							...
							if (in_array($type, $allowedExts)){			
								if ($error > 0){
									...
									$sql = "insert into file_list (true_name,new_name,up_time,who_share,course_id,ins) 
															values ('$tmp','$newName',CURDATE(),'$s_name','$course_id','$ins')";															
									$isUp = upload($sql);	
									//判断上传是否结束或成功
									if($isOk&&$isUp){											
											echo  "<script>					
													window.location.href='resources.php?pageNum=1&&action=list&&snumber=".$snumber."&&course_id=".$course_id."';
														alert('资源共享成功');
													</script>"
													;																																
